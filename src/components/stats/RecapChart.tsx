...
export default function RecapChart() {
  ...
  const chartHeight = Math.max(300, data.length * 25);
    ...
    <div className="w-full bg-[#1c252e] border border-gray-700 rounded-lg p-6 overflow-hidden" style={{ height: `${chartHeight}px` }}>
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ left: 10 }}>
          ...
          <YAxis dataKey="agent_name" type="category" stroke="#9ca3af" fontSize={10} width={40} />
          <Tooltip 
            content={<CustomTooltip />}
            cursor={{ fill: 'rgba(255, 255, 255, 0.05)' }}
          />
          <Bar dataKey="pick_count" fill="#FF4655" radius={[0, 4, 4, 0]} barSize={12}>
             {data.map((_, index) => (
